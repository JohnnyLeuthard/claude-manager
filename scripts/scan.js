...
<body>
  ...
  <footer>Generated by <strong>claude-manager</strong> &mdash; ${escapeHtml(now)}${(() => {
    const reportsDir = path.join(__dirname, '..', 'reports');
    try {
      const audits = fs.readdirSync(reportsDir).filter(f => f.startsWith('audit-') && f.endsWith('.html'));
      const latest = audits.sort().pop();
      if (latest) {
        const auditPath = path.join(reportsDir, latest);
        return ` &nbsp;|&nbsp; Last audit: <a href="file://${escapeHtml(auditPath)}" target="_blank" rel="noopener">${escapeHtml(latest)} &rarr;</a>`;
      }
    } catch (e) {}
    return '';
  })()}</footer>
