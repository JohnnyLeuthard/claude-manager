...
`audit.js` is a **separate script** from `scan.js`, following the same dual-output pattern (terminal ANSI + optional `--html`). The two scripts have different cadences: scan runs often to check sizes; audit runs deliberately when concerned about secrets. Keeping them separate avoids slowing down the dashboard with file-content scanning.

The audit HTML report is always **static** — no live server, no `/rescan` endpoint. Secrets are never printed in full; all matched values are truncated.

### CLI Flags

| Flag | Behavior |
|------|----------|
| _(no flags)_ | Terminal output only, ANSI color if TTY |
| `--html` | Terminal output + write `reports/audit-YYYY-MM-DD.html` |
| `--html-only` | Write HTML only, suppress terminal output |
| `--no-color` | Disable ANSI color in terminal output |
...
1. Parse args
2. `getClaudeDir()` → `~/.claude`
3. `runAudit(claudeDir)` → array of finding objects
4. Compute severity summary (HIGH count, WARN count, OK count)
5. If `showTerminal`: `renderTerminal(findings, summary)`
6. If `showHtml`: `renderHTML(findings, summary)` → write to `reports/audit-YYYY-MM-DD.html`

### `runAudit(claudeDir)` — Scan Targets

**Target 1: `shell-snapshots/`**
- Read each `.json` file in the folder
- Convert to string, run regex patterns against full content
- Patterns (case-insensitive):
  - `sk-[a-z0-9]{20,}` — OpenAI / Anthropic API key
  - `ghp_[a-zA-Z0-9]{36}` — GitHub personal access token
  - `AKIA[0-9A-Z]{16}` — AWS access key
  - `Bearer [a-zA-Z0-9._-]{20,}` — Bearer auth token
  - `[A-Z_]+(API_KEY|TOKEN|SECRET|PASSWORD)\s*=\s*\S+` — generic named secret
- For each match: record file path, pattern name, truncated value (first 6 chars + `***`)
- Severity: **HIGH**

**Target 2: `session-env/`**
- Read each file in the folder (any format)
- Scan for environment variable names matching sensitive patterns
- Patterns: `[A-Z_]*(KEY|TOKEN|SECRET|PASSWORD|PASS|DATABASE_URL)[A-Z_]*\s*=`
- Report variable names only — not values (values may not even be present)
- Severity: **WARN**

**Target 3: `mcp-needs-auth-cache.json`**
- Parse JSON, extract all service host/domain entries
- Compare against known-safe list: `anthropic.com`, `claude.ai`, `github.com`, `google.com`, `linear.app`
- Any host not on the list → WARN with the host name
- All recognized → OK
- Severity: **WARN** (unusual) or **OK** (all recognized)
...
```javascript
{
  ...
  target:   String,  // 'shell-snapshots/', 'session-env/', 'mcp-needs-auth-cache.json'
  file:     String,  // filename within target, or null for single-file targets
  pattern:  String,  // pattern name that matched, or summary description
  preview:  String,  // truncated: "sk-proj***" or variable name or host
  action:   String,  // 'rotate credential + delete file' | 'review' | 'all clear'
}
```

### Secret Truncation Rule

**Never log the full value of a matched secret.** Always truncate to first 6 chars + `***`:
...
Applies to both terminal and HTML output.

### Severity Levels

| Level | Symbol | Meaning |
|-------|--------|---------|
| HIGH | 🔴 | Likely credential or secret — rotate immediately, consider deleting the file |
| WARN | 🟡 | Sensitive name or unusual service — review, no immediate action required |
| OK | 🟢 | Nothing concerning found in this target |

### HTML Report Layout (`audit-YYYY-MM-DD.html`)

Self-contained (no external CSS/JS, no CDN). Layout top to bottom:

1. `<header>` — "Security Audit Report" + date
2. `.summary` — stats bar: HIGH count / WARN count / OK count / targets scanned
3. `.findings-list` — one card per finding (HIGH first, then WARN, then OK)
4. `<footer>` — "Generated by claude-manager · Run again: `node scripts/audit.js --html`"

Finding cards follow the same visual pattern as `scan.js` dashboard cards: left border color-coded by severity, severity badge, target name, file path, pattern name, truncated preview, recommended action.

### scan.js Integration

After `audit.js` is built, `scan.js` HTML footer gains a "Last audit" line:

```
Generated by claude-manager · {date}  |  Last audit: audit-2026-05-25.html → [View →]
```

The link is only rendered if at least one `audit-*.html` file exists in `reports/`. `scan.js` finds it with:

```javascript
const audits = fs.readdirSync(reportsDir).filter(f => f.startsWith('audit-') && f.endsWith('.html'));
const latest = audits.sort().pop(); // lexicographic sort works for YYYY-MM-DD filenames
```
